...
export class BlogFService {
  ...
  createBlog(blog_title: string, blog_body: string,): Observable<any> {
    const blogData = {blog_title, blog_body};
    const token = this.cookieService.get('token');
    const headers = new HttpHeaders().set('Authorization', `Bearer ${token}`);

    return this.http.post<any>(`${this.apiURL}create-blog`, blogData, {headers, withCredentials: true});
  }
  ...
  displayBlogById(blog_id: number): Observable<any> {
    return this.http.get<any>(`${this.apiURL}blogs/${blog_id}`, {withCredentials: true});
  }
